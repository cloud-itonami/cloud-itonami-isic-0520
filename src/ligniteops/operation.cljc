(ns ligniteops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (LigniteOpsAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the LigniteMiningGovernor (:govern)
  ...
  (:require [langgraph.graph :as g]
            ...
            [ligniteops.governor :as governor]
            [ligniteops.phase :as phase]
...
(defn build
  ...
  `ligniteops.store/Store`).
  opts:
    :advisor      -- a `ligniteops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; LigniteOpsAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; LigniteMiningGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
